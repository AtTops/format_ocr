...
if __name__ == '__main__':
    ...
    _, result, angle = core.model(img,
                                  detect_angle=False,  # 是否进行图片/文字方向检测
                                  ...
                                  if_adjust_degree=False  # 前提是detect_angle为True(TODO : wrong, 为False时，框与图都不旋转)
                                  )
    ...
    img2 = Image.open(paths[3]).convert("RGB")
    t2 = time.time()
    _, result, angle = core.model(img2,
                                  detect_angle=False,  # 是否进行文字方向检测
    ...
    img3 = Image.open(paths[3]).convert("RGB")
    t3 = time.time()
